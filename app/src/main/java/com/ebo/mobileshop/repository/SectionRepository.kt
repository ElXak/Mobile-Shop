...
class SectionRepository(val app: Application) {
    ...
    private val _data = MutableLiveData<List<SelectedSection>>()
    val data: LiveData<List<SelectedSection>> = _data

    // for passing data to layout
    private val _selectedData = MutableLiveData<SelectedSection>()
    val selectedData: LiveData<SelectedSection> = _selectedData

    // instance of DAO
    private val dao = MobileShopDb.getDatabase(app).sectionDao()
    ...
    suspend fun webService() {
        if (Network.isAvailable(app)) {
            ...
            // get service data
            val serviceData = service.getSections().body() ?: emptyList()

            // update data in sqlLite database
            dao.deleteAll()
            dao.insertAll(serviceData)

            val data = dao.getRoot()
