...
import com.ebo.mobileshop.ITEM_ID
import com.ebo.mobileshop.SECTION_ID
...
class ItemDetailsRepository(val app: Application, private val params: Map<String, Int>) {
...
*/
    ...
    suspend fun webService() {
        if (Network.isAvailable(app)) {
            ...
            // get service data
            val serviceData = service.getItemDetails(
                params[ITEM_ID]!!,
                params[SECTION_ID]!!
            ).body()

            // update data in sqlLite database
            dao.insert(serviceData!!)

            val data = dao.getById(
                params[ITEM_ID]!!,
                params[SECTION_ID]!!
            )
